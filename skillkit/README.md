...
说明：downloads 是平台抓取与榜单曝光的计数，不是用户行为（实测：全员约 +4/天，在榜技能另加数百/天）；真实采用看 installs；stars 是收藏计数，不是 1–5 星评分；浏览量平台不提供。
```
...
| 上架很久没量，热榜技能一天涨几千 | `downloads` = 全员抓取基线（约 +4/天）+ **榜单曝光**（在榜中位 +340/6h，不在榜 +1/6h）；与质量分、触发面、上架时长无关，hot 榜按下载排序、回路自我强化 | **实测（四天，321 组配对 0 例外 + 同窗口对照）** | `stats` 把 downloads 当曝光计数报；真实采用看 `installs` |

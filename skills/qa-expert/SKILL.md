---
name: qa-expert
description: 测试专家.Skill——覆盖一名测试工程师的整份工作，服务对象是测试工程师、QA，以及要自测再提测的开发。只要用户的问题涉及「这东西怎么测、测到什么程度算够」，无论是新需求要出测试策略与用例、接口改了要做契约与回归、流水线里用例时灵时不灵、覆盖率报告看不出该补哪里，还是要压测、要上线前验收、要把一个偶现缺陷复现出来，都从这里进。当用户说 测试、测一下、用例、case、测试点、边界值、等价类、冒烟、回归、自测、提测、验收、UAT、接口测试、契约测试、Mock、断言、flaky、随机失败、CI 挂了、覆盖率、coverage、补测试、压测、性能测试、QPS、并发、缺陷、bug、复现、偶现、提单、测试报告、质量门禁、上线前检查 等任一说法时使用；本技能先判断处在测试的哪一环，再给方法并路由到精专子技能。不做：替人点「通过」、替人下能不能上线的结论。
author: Captain
version: 0.1.0
display_name: "测试专家.Skill"
display_name_en: "QA Expert.Skill"
description_zh: "一个入口覆盖测试全流程：测试策略与用例设计、接口契约与回归、不稳定用例治理、覆盖率缺口、压测、上线前验收、缺陷分析与复现；按所处环节路由到精专子技能。"
description_en: "One entry point for the whole QA loop—test strategy and case design, API contracts and regression, flaky-test triage, coverage gaps, load testing, pre-release acceptance, defect analysis and reproduction; routes to focused sub-skills."
tags:
  - "测试"
  - "QA"
  - "用例设计"

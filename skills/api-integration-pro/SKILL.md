...
description: 接口联调助手，覆盖前后端、第三方与内部服务对接过程里的全部琐事：接口调试与调不通排查、curl 与浏览器抓包转代码、接口没文档怎么办、OpenAPI 转中文文档、改接口会不会把调用方搞挂的兼容性检查、契约回归与冒烟、两个环境返回不一致的差分对比、JSON 响应逐字段比对、mock 数据与字段契约推断、401 / 403 / token 过期 / 签名不对的鉴权排错、TLS 握手与证书报错、域名解析没生效、接口扛不扛得住的压测。当用户说联调、对接、接口、调接口、调不通、返回不对、字段、跨域、CORS、鉴权、token、JWT、签名、Swagger、OpenAPI、Postman、curl、抓包、mock、契约、回归、压测、QPS、超时、证书、DNS 等任一说法，但不确定该用哪个专门技能时使用；本技能判断意图，给出对应方法并路由到精专子技能。不做：替用户保管或填写任何真实凭据。
author: Captain
version: 0.1.0
display_name: "接口联调助手"
display_name_en: "API Integration Pro"
description_zh: "一个入口覆盖接口对接全流程：抓包转代码、接口文档生成、破坏性变更检查、契约回归、环境差分、JSON 比对、mock 与字段推断、鉴权排错、DNS 与证书、压测；按意图路由到精专子技能。"
description_en: "One entry point for API integration work: curl to code, doc generation from OpenAPI, breaking-change checks, contract regression, cross-environment diffing, JSON comparison, mocks and schema inference, auth debugging, DNS and TLS, load testing; routes to focused sub-skills."
tags:
  - "接口联调"
  - "API 调试"
  - "OpenAPI"

...
## 何时用
对方说没听懂、要求用大白话再说一遍、或者你判断上一条回复术语太重/太绕时用。对方是要新增信息或换个话题，不是"没听懂"，就正常回答，不要套这套重讲流程。
...
[重讲正文：短句、一句一意、主动语态，只用统一语言表里的词]

一句话：[复述结论]
```

不产出新增信息，只产出同一件事的更简单表达。

## 边界
禁止事项：不加新信息——重讲的是同一件事，想补充的留到之后单独说；不换说法——一个概念全程用同一个词；术语只用项目统一语言表里的词，没有就用日常语言解释，不做发明新词这种事。

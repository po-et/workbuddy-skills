...
- 质量巡检把 34 个低分技能修到 ≥ 80 并升 0.1.1 重发（更新沿用原 skillId）；发布限流的真实规律：据观察推测（需确认）：SkillHub 每账号每天 100 次成功发布上限——2026-09-17 当天恰好第 100 次成功后，此后所有发布（含未改动的对照技能）一律报「发布频率过高」，与间隔无关；本地 00:48 仍未恢复，可能按 UTC 日或滚动 24 h 计。对策：每天不超过 90 次，更新版分天发。

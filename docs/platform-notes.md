...
### 单账号 200 个上限（2026-09-26 实测）

- 个人账号的社区技能**上限 200 个**。满额后再发新 slug，CLI 返回 `权限不足: 已达社区 Skill 发布上限（200 个），请删除不需要的 Skill 后再发布`；网页端的错误码是 `SKILL_LIMIT_REACHED`，提示「已发布 used/limit 个」。
- **已有技能发新版本不受影响**：满额后当晚的升级发布都成功了（第一个是 23:27「请假条」0.1.1）。
- 个人技能在网页上只有「删除」，没有「下架」。确认框原文是「删除后数据将无法恢复，此操作不可撤销。删除后该名称会被释放」。团队/组织空间另有「下架/上架」，那是组织管理员的功能。CLI 没有删除命令。
- 数自己有多少个：`GET /api/v1/users/<handle>/skills?page=N&pageSize=100` 翻页累加。handle 是 `user_xxxxxxxx` 这种形式，不是命名空间名。`pageSize` 超过 100 时会退回默认的 20 条。
- 对策：把 200 个名额当成货架。新技能只发有真实需求的词（搜索空缺 + 当季）；只有抓取基线下载、评测分又低的技能，由账号本人在网页上删掉腾位。审计方法见 `docs/metrics/slot-audit-2026-09-26.md`。

...
6. **要上开放平台的技能，frontmatter 还必须有**：`version`、`display_name`、`display_name_en`、`description_zh`、`description_en`（平台解析器硬性要求）；建议再加 `examples_zh` / `examples_en`，各**最多 3 条**（实测见 docs/platform-notes.md）
